...
//  setMic(dBAAverage);
  ...
  Debug("SPL_dBA: %.1f, SPL_peak_mPa: %s, dBA peak: %.1f, dBA average: %.1f\r\n", dBA,mPabuffer, dBAmax, dBAAverage);
...
MicrophoneState Mic_Upkeep(){
  ...
  case MIC_STATE_WAIT_FOR_COMPLETION:
    if (getSoundData(&soundData, true, true)) {
      ...
      dB->measurementValue = dBValue;
      dB->measurementDone = true;
      MIC_Print();
      if (!startSPLcalculation()) {
        errorHandler(__func__, __LINE__, __FILE__);
      }
      
      MicState = MIC_STATE_DONE;//MIC_STATE_WAIT;
      dBAResume = false;
    }
    break;
  case MIC_STATE_DONE:

...
// OK
bool aBuff2int(char *aBuff, uint8_t start, uint8_t stop, uint8_t *value)
{
	char iBuff[5];
	uint8_t i;
	uint8_t pos = 0;
	...
	// Check if iBuff contains number
	char *s = iBuff;
	while (*s == ' ')
	{
		s++;
...
void ParseTime(char *buffer)
{
	...
	RTC_GetTime(&currentTime, &currentDate);
	if (   strlen(buffer) >= 37
		&& aBuff2int(buffer, 24, 25, &currentTime.Hours)
		&& aBuff2int(buffer, 27, 28, &currentTime.Minutes)
		&& aBuff2int(buffer, 30, 31, &currentTime.Seconds)
		&& aBuff2int(buffer, 35, 36, &currentDate.Year) && currentDate.Year >= 2025
		&& aBuff2int(buffer, 17, 19, &currentDate.Month)
		&& aBuff2int(buffer, 21, 22, &currentDate.Date)
		&& aBuff2int(buffer, 13, 15, &currentDate.WeekDay))
	{
		Info("Current RTC time before update is: %02dh:%02dm:%02ds", currentTime.Hours, currentTime.Minutes, currentTime.Seconds);
		Info("Current RTC date before update is: %02d-%02d-%02d", currentDate.Date, currentDate.Month, currentDate.Year);
...
// Functie om een alarm in te stellen
// void RTC_SetAlarm(uint32_t intervalSeconds)
// {

// 	uint32_t nextAlarmPosixTime = getPosixTime() + intervalSeconds;
// 	RTC_DateTypeDef nextAlarmDate;
// 	RTC_TimeTypeDef nextAlarmTime;

// 	breakPosixTime(nextAlarmPosixTime, &nextAlarmDate, &nextAlarmTime);

// 	RTC_AlarmTypeDef sAlarm = {0};

// 	sAlarm.AlarmTime.Hours = nextAlarmTime.Hours;
// 	sAlarm.AlarmTime.Minutes = nextAlarmTime.Minutes;
// 	sAlarm.AlarmTime.Seconds = nextAlarmTime.Seconds;
...
void watchdogStopMode(uint16_t sleepTime){
	uint16_t numberOfSleeps = sleepTime / SAFE_SLEEP_TIME;
	uint16_t residualSleepTime = sleepTime % SAFE_SLEEP_TIME;
	for(uint16_t i = 0; i < numberOfSleeps; i++){
		petDog();
		Enter_Stop_Mode(SAFE_SLEEP_TIME);
	}
	petDog();
	if(residualSleepTime != 0){
		Enter_Stop_Mode(residualSleepTime);
	}
	petDog();
...
void InitClock(RTC_HandleTypeDef *h_hrtc)
{
	...
	// HAL_RTCEx_SetWakeUpTimer_IT(h_hrtc, FIRST_SLEEP_TIME, RTC_WAKEUPCLOCK_CK_SPRE_16BITS);
}
